    ...
    : QObject(client)
{
    ...
    for (const char *varname : envVariables) {
       ...
       // printf("%s:%s\n",varname, username.toLocal8Bit().constData());
        if (!username.isNull())
            break;
    }
...
void PeerManager::updateAddresses()
{
    ...
    for (const QNetworkInterface &interface : interfaces) {
        ...
        for (const QNetworkAddressEntry &entry : entries) {
            ...
            //printf("BroadcastAddresses: %s ipAdresses: %s\n",broadcastAddress.toString().toLocal8Bit().constData(), entry.ip().toString().toLocal8Bit().constData());
