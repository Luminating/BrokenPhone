 ...
 *  ]
 *  command     = plaintext / ping / pong / greeting / image
 *  plaintext   = { 0 => text }
 *  ping        = { 1 => null }
 *  pong        = { 2 => null }
 *  greeting    = { 3 => text }
    ...
    while (reader.lastError() == QCborError::NoError) {
        if (state == WaitingForGreeting) {
            ...
            if (reader.isString()) {
                ...
                buffer += r.data;
                //printf("command payload String: %s\n", r.data.toLocal8Bit().constData());
                ...
                //printf("ByteArray size processReadyRead =%d status =%d\n", bufferArray.size(), r.status);
                if (r.status != QCborStreamReader::EndOfString) {
                    continue;
                }
